...
class DependencyRuleValidatorFullTest {
  ...
  void testFromLevelsAndNullEdgeCases() {
    ...
    DependencyEdge assoc =
        new DependencyEdge("domain.A", "adapter.B", DependencyEdge.Kind.ASSOCIATION, null, false);
    assertFalse(v.evaluate(assoc).isViolating());

    // Edge where target is unranked
    DependencyEdge unrankedTarget =
        new DependencyEdge("domain.A", "unranked.B", DependencyEdge.Kind.DEPENDENCY, null, false);
    assertFalse(v.evaluate(unrankedTarget).isViolating());

    // Edge where source is unranked
    DependencyEdge unrankedSource =
        new DependencyEdge("unranked.A", "domain.B", DependencyEdge.Kind.DEPENDENCY, null, false);
    ...
    DependencyEdge outerToInner =
        new DependencyEdge("outer.A", "inner.B", DependencyEdge.Kind.DEPENDENCY, null, false);
    assertFalse(v2.evaluate(outerToInner).isViolating());

    // Edge where fromRank == toRank (same rank, not violating)
    DependencyEdge sameRank =
        new DependencyEdge("inner.A", "inner.B", DependencyEdge.Kind.DEPENDENCY, null, false);
    assertFalse(v2.evaluate(sameRank).isViolating());

    // Edge where fromRank < toRank (inner -> outer, VIOLATING)
    DependencyEdge innerToOuter =
        new DependencyEdge("inner.A", "outer.B", DependencyEdge.Kind.DEPENDENCY, null, false);
  ...
  void testFromProposal() {
    ...
    Proposal.ProposalLayer layer1 =
        new Proposal.ProposalLayer("core", "Core", List.of("com.app.core"));
    Proposal.ProposalLayer layer2 =
        new Proposal.ProposalLayer("infra", "Infra", List.of("com.app.infra"));
