...
public class FileMailboxService {
  ...
  public synchronized MailboxEnvelope.MailboxCommand appendCommand(
    ...
    MailboxEnvelope.MailboxCommand cmd =
        new MailboxEnvelope.MailboxCommand(newId, op, target, payload);
  ...
  public synchronized MailboxEnvelope.MailboxCommand popOldest(
    ...
    List<MailboxEnvelope.MailboxCommand> remaining =
        new ArrayList<>(current.queue().subList(1, current.queue().size()));
